...
namespace Celeste.Mod.MoreLockBlocks.Entities
{
    ...
    internal class DreamLockBlockV2 : DreamBlock
    {
        ...
        private bool CanDashThrough
        {
            get
            {
                if (MoreLockBlocksModule.Session.DreamBlockDummyStates.TryGetValue(component.ID, out bool value))
                    return value;
                else
                {
                    bool state = MoreLockBlocksModule.Session.DreamBlockDummyStates[component.ID] = false;
                    if (ignoreInventory)
                    {
            ...
            set
            {
                MoreLockBlocksModule.Session.DreamBlockDummyStates[component.ID] = value;
                if (ignoreInventory)
                {
